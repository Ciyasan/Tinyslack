...
public class ChannelController {
  ...
  public ResponseEntity<Channel> getChannelByName(
  ...
  public ResponseEntity<List<Message>> getMessagesByChannelName(
    @PathVariable("name") String channelName
  ) {
    Optional<Channel> channel = channelService.getOneChannelByName(channelName);
    ...
    Channel channelToGet = channel.get();
    List<Message> messagesFromChannel = messageService
      .getAllMessages()
      .stream()
  ...
  public ResponseEntity<Object> saveChannel(@RequestBody Channel newChannel) {
    Optional<Channel> channelToPost = channelService.getOneChannelByName(
  ...
  public ResponseEntity<Object> deleteChannel(
    ...
    Optional<Channel> channelToDelete = channelService.getOneChannelByName(
      channelName
    );
    ...
    if (channelToDelete.get().getLocked() == 1) {
      return ResponseEntity
        .badRequest()
        .body("Le canal est verouillé, suppression impossible");
    }
  ...
  public ResponseEntity<Object> putChannel(
    @PathVariable("name") String channelName,
    @RequestBody Channel modification
  ) {
    // pour récupérer le canal qui correspond au "name" en URL
    Optional<Channel> channelToPut = channelService.getOneChannelByName(
      channelName
    );
    ...
    if (!channelId.equals(modification.getChannelId())) {
      return ResponseEntity
        .badRequest()
        .body("Ce canal ne correspond pas à la modification demandée");
    }
    ...
    if (channelToPut.get().getLocked() == 1) {
      return ResponseEntity
        .badRequest()
        .body("Ce canal est vérouillé et ne peut être modifié");
    }
  ...
  public ResponseEntity<Object> patchChannel(
    @PathVariable("name") String channelName,
    @RequestBody Channel patch
  ) {
    Optional<Channel> channelToPatch = channelService.getOneChannelByName(
      channelName
    );

    Optional<Channel> channel = channelService.getOneChannelByName(
      patch.getChannelName()
    );
    ...
    if (!channelToPatch.get().getChannelId().equals(patch.getChannelId())) {
      return ResponseEntity
        .badRequest()
        .body("Ce canal ne correspond pas à la modification demadnée");
    }
